...
def load_and_filter_data(csv_path: Path, k_filter: int = 3) -> pd.DataFrame:
    ...
    # RECALCULATE AS COST RATIO (heuristic/exhaustive)
    df_filtered['optimality'] = np.where(
        df_filtered['optimal_cost'] > 0,
        df_filtered['heuristic_cost'] / df_filtered['optimal_cost'],
        np.where(df_filtered['heuristic_cost'] == 0, 1.0, np.nan)
    )
...
def print_summary_statistics(df: pd.DataFrame, k: int):
    ...
    if not df_finite.empty:
        mean_val = df_finite['optimality'].mean()
        median_val = df_finite['optimality'].median()
        std_val = df_finite['optimality'].std()
        min_val = df_finite['optimality'].min()
        max_val = df_finite['optimality'].max()
        
        print(f"\nCost Ratio Statistics (finite values only):")
        ...
        print(f"\n  Perfect matches (ratio = 1.0): {n_perfect}/{len(df_finite)} "
              f"({n_perfect/len(df_finite)*100:.1f}%)")
        
        # Statistics by overhead
        print(f"\nBreakdown by Overhead:")
        print(f"  {'Overhead':<12} {'Count':<8} {'Mean Ratio (H/E)':<20} {'Std':<12} {'Perfect %':<12}")
        print(f"  {'-'*75}")
        for oh in sorted(df['overhead'].unique()):
            df_oh = df_finite[df_finite['overhead'] == oh]
            if len(df_oh) > 0:
                mean_opt = df_oh['optimality'].mean()
                std_opt = df_oh['optimality'].std()
                n_perf = (df_oh['optimality'] == 1.0).sum()
                perf_pct = n_perf / len(df_oh) * 100
                print(f"  {oh:<12.2f} {len(df_oh):<8} {mean_opt:<20.3f} {std_opt:<12.3f} {perf_pct:<12.1f}%")
    else:
...
def create_plot(df: pd.DataFrame, output_path: Path, k: int):
    ...
    # Prepare data for box plot (raw ratio, not percentage)
    data_by_overhead = [df_finite[df_finite['overhead'] == oh]['optimality'].values 
                        for oh in overhead_values]
    
    # Print statistics for each overhead
    print(f"\nCost ratio distribution by overhead:")
    for oh, data in zip(overhead_values, data_by_overhead):
        print(f"  Overhead {oh:.2f}: median={np.median(data):.3f}, "
              f"mean={np.mean(data):.3f}, n={len(data)}")
    ...
    # Labels and title
    ax.set_xlabel('Network Overhead', fontsize=18, fontweight='bold')
    ax.set_ylabel('Cost Ratio (H/E)', fontsize=18, fontweight='bold')
    # ax.set_title('Heuristic Performance vs Network Overhead\n(1.0 = Equal Cost, Lower is Better)', 
    ...
    # Add horizontal line at 1.0 (equal cost)
    ax.axhline(y=1.0, color='green', linestyle='--', linewidth=2, alpha=0.7, 
               label='1.0 (Equal cost)', zorder=10)
    
    # Add horizontal line at 0.5 
    ax.axhline(y=0.5, color='orange', linestyle='--', linewidth=1.5, alpha=0.5, 
               label='0.5 (Heuristic 2× cheaper)', zorder=10)
    
    # Set y-axis range
    y_min = max(0, df_finite['optimality'].min() - 0.05)
    y_max = min(1.1, df_finite['optimality'].max() + 0.05)
...
def create_circuit_size_plot(df: pd.DataFrame, output_path: Path, k: int, overhead_value: float = 1.0):
    ...
    print(f"  Mean cost ratio (H/E): {df_plot['optimality'].mean():.3f}")
    
    # Create figure
    fig, ax = plt.subplots(figsize=(10, 7))
    
    # Scatter plot - y-axis is heuristic_cost, color is cost ratio
    scatter = ax.scatter(df_plot['qubits_int'], 
                        df_plot['heuristic_cost'],
                        alpha=0.6, 
                        s=100,
                        c=df_plot['optimality'],
                        cmap='RdYlGn_r',
                        vmin=0,
                        vmax=2.0,
                        edgecolors='black',
    ...
    cbar.set_label('Cost Ratio (H/E)', fontsize=12, fontweight='bold')
    ...
    # Labels and title
    ax.set_xlabel('Circuit Size (Number of Qubits)', fontsize=18, fontweight='bold')
    ax.set_ylabel('Heuristic Cut Cost', fontsize=18, fontweight='bold')
...
def main():
    ...
    print("COST RATIO VIEW: Heuristic / Exhaustive (lower is better)")
    print("="*70 + "\n")
