...
GOLDEN = {
    ...
    "Teleportation": {"cut_cost": 4, "sizes": [3, 4, 4], "global_parallelism": 0, "parallelism_at_t3": 0},
...
def check_hdh_outputs(hdh_graph, label):
    ...
    assert cost(hdh_graph, partitions) == GOLDEN[label]["cut_cost"]
    assert partition_sizes(partitions) == GOLDEN[label]["sizes"]
    assert compute_parallelism_by_time(hdh_graph, partitions, mode="global") == GOLDEN[label]["global_parallelism"]
