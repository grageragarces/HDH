...
def brute_force_qubit_level_timed(
    ...
    Returns:
        (best_partition, best_cut_cost, partitions_checked, completed)
    ...
    for assignment in assignments:
        ...
        if current_time - last_progress_time >= progress_interval:
            ...
            print(f"  ⏳ {int(elapsed)}s elapsed | "
                  ...
                  f"Rate: {rate:.0f}/s | "
                  f"Best: {min_cut if min_cut != float('inf') else 'N/A'} | "
        ...
        if cuts < min_cut:
            ...
            print(f"  🎯 NEW BEST: {min_cut} cuts (found at {int(elapsed_now)}s, {partitions_checked:,} checked)")
